...
function Test-CrudApiManagement {
    ...
    try {
		...
		# enable TLS and 3DES CipherSuites
		$sslSetting = New-AzApiManagementSslSetting -FrontendProtocols $enableTls -CipherSuites $enable3DES
        # Create API Management service
        $result = New-AzApiManagement -ResourceGroupName $resourceGroupName -Location $location -Name $apiManagementName -Organization $organization -AdminEmail $adminEmail -SslSettings $sslSetting
		...
		Assert-NotNull $result.SslSettings
		Assert-AreEqual "True" $result.SslSettings.FrontendProtocols["Tls10"]
