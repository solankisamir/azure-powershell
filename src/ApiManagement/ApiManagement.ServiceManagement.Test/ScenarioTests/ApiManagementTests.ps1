...
function ApiSchema-SwaggerCRUDTest {
    ...
    try {
        ...
        $apiSchema = New-AzApiManagementApiSchema -Context $context -ApiId $newApiId -SchemaId $newSchemaId -SchemDocumentContentType swaggerdefinition `
        ...
        # set api Schema
        $apiSchema = Set-AzApiManagementApiSchema -InputObject $getApiSchema -SchemDocumentContentType "application/json" -PassThru
        Assert-NotNull $apiSchema
...
function ApiSchema-WsdlCRUDTest {
    ...
    try {
        ...
        $apiSchema = New-AzApiManagementApiSchema -Context $context -ApiId $newApiId -SchemaId $newSchemaId -SchemDocumentContentType xsdschema `
...
function Diagnostic-CrudTest {
    ...
    try {
        ...
        $diagnostic.BackendSettingSetting = $null
...
function ApiDiagnostic-CrudTest {
    ...
    try {
        ...
        Assert-NotNull $apiDiagnostic.Sampling
        Assert-AreEqual fixed $apiDiagnostic.SamplingSetting.SamplingType
        Assert-NotNull $apiDiagnostic.Frontend
        ...
        Assert-NotNull $apiDiagnostic.Backend
        Assert-AreEqual 'Content-Type' $apiDiagnostic.Backend.Request.Headers[0]
        Assert-AreEqual 'UserAgent' $apiDiagnostic.Backend.Request.Headers[1]
        Assert-AreEqual 'Content-Type' $apiDiagnostic.Backend.Response.Headers[0]
        Assert-AreEqual 'UserAgent' $apiDiagnostic.Backend.Response.Headers[1]

        # remove pipeline diagnostic from backend and reduce Sampling percentage to 50%
        $apiDiagnostic.Backend = $null
        $apiDiagnostic.SamplingSetting.SamplingPercentage = 50
        $updateDiagnostic = Set-AzApiManagementDiagnostic -InputObject $apiDiagnostic -ApiId $apis[0].ApiId -PassThru

        Assert-NotNull $updateDiagnostic        
        Assert-AreEqual $newDiagnosticId $updateDiagnostic.DiagnosticId
        Assert-Null $updateDiagnostic.Backend
        Assert-AreEqual fixed $updateDiagnostic.SamplingSetting.SamplingType
        Assert-AreEqual 50 $updateDiagnostic.SamplingSetting.SamplingPercentage        
        Assert-NotNull $updateDiagnostic.Frontend
        Assert-AreEqual 'Content-Type' $updateDiagnostic.FrontendSetting.Request.HeadersToLog[0]
        Assert-AreEqual 'UserAgent' $updateDiagnostic.FrontendSetting.Request.HeadersToLog[1]
        Assert-AreEqual 'Content-Type' $updateDiagnostic.FrontendSetting.Response.HeadersToLog[0]
        Assert-AreEqual 'UserAgent' $updateDiagnostic.FrontendSetting.Response.HeadersToLog[1]
